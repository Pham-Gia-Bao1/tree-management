...
export default function Diagram() {
    ...
    const fetchMemberDetail = useCallback(async (memberId: string) => {
        setDrawerLoading(true); setViewingMemberId(memberId);
        try {
        ...
        } catch { message.error("Lỗi kết nối"); } finally { setDrawerLoading(false); }
    ...
    const handleCreateDiscipleLine = async () => {
        try {
            ...
            const json = await res.json();
            if (json.success) {
                message.success("Thêm thành công!"); createForm.resetFields(); setLeftPanelOpen(false);
                const reload = await fetch(`/api/discipleship-tree?courseId=${selectedCourse}`).then(r => r.json());
                if (reload.success) { setTreeLinks(reload.data.links); setMemberMap(new Map(reload.data.members?.map((m: MemberProfileRecord) => [m.id, m]) || [])); }
            } else message.error(json.error?.message || "Lỗi");
        } catch (err) { if ((err as any)?.errorFields) return; message.error("Lỗi kết nối"); } finally { setCreatingLink(false); }
    };
    ...
    const renderRightPanel = () => {
        ...
        if (drawerLoading) return <Flex vertical gap={16} className="p-4"><Skeleton active paragraph={{ rows: 6 }} /></Flex>;
                ...
                <Card size="small" title={<span className="text-slate-700"><Mail size={14} className="inline mr-2"/> Tổng quan</span>} className="shadow-none border-slate-200">
                    <div className="space-y-2">
                        <div className="flex items-center gap-2">
                            ...
                            <div><Text strong>{data.member.fullName}</Text><br/><Text type="secondary" style={{ fontSize: 12 }}>{data.member.email || "-"}</Text></div>
                        </div>
                    </div>
                </Card>
